...
def past_regular():
    ...
    # starting arcs
    for char in markers:
        f_reg.add_arc('start', 'marker', char, [char])

    # voiced arcs
    for char in voiced:
        f_reg.add_arc('voiced', 'voiced', char, [char])
    
    for char in voiceless:
        f_reg.add_arc('voiced', 'voiceless', char, [char])
    
    for char in alveolar:
        f_reg.add_arc('voiced', 'alveolar', char, [char])

    for char in markers:
        f_reg.add_arc('voiced', 'marker', char, [char])

    # voiceless arcs
    for char in voiceless:
        f_reg.add_arc('voiceless', 'voiceless', char, [char])
    
    for char in voiced:
        f_reg.add_arc('voiceless', 'voiced', char, [char])
    
    for char in alveolar:
        f_reg.add_arc('voiceless', 'alveolar', char, [char])
    
    for char in markers: 
        f_reg.add_arc('voiceless', 'marker', char, [char])

    # alveolar arcs
    for char in alveolar:
        f_reg.add_arc('alveolar', 'alveolar', char, [char])
    
    for char in voiced:
        f_reg.add_arc('alveolar', 'voiced', char, [char])

    for char in voiceless:
        f_reg.add_arc('alveolar', 'voiceless', char, [char])

    for char in markers:
        f_reg.add_arc('alveolar', 'marker', char, [char])

    # marker arcs
    for char in markers:
        f_reg.add_arc('marker', 'marker', char, [char])

    for char in voiced:
        f_reg.add_arc('marker', 'voiced', char, [char])
   
    for char in voiceless:
        f_reg.add_arc('marker', 'voiceless', char, [char])
    
    for char in alveolar:
        f_reg.add_arc('marker', 'alveolar', char, [char])

    # ending arcs
    f_reg.add_arc('voiced', 'end', '#', ['d', '#'])
    f_reg.add_arc('voiceless', 'end', '#', ['t', '#'])
    f_reg.add_arc('alveolar', 'end', '#', ['ɪ', 'd', '#'])

    return f_reg

def past_irreg():

    f_irreg = FST('past_irreg')
    ...
    f_irreg.add_arc('z_əraɪ', 'end', '#', ['ə', 'ɹ', 'o', 'ʊ', 'z', '#'])
    ...
    f_irreg.add_arc('k_əweɪ', 'end', '#', ['ə', 'w', 'o', 'ʊ', 'k', '#'])
    ...
    f_irreg.add_arc('t_baɪ', 'end', '#', ['b', 'ɪ', 't', '#'])
    ...
    f_irreg.add_arc('ʊ_blo', 'end', '#', ['b', 'l', 'u', '#'])
    ...
    f_irreg.add_arc('k_bɹeɪ', 'end', '#', ['b', 'ɹ', 'o', 'ʊ', 'k', '#'])
    ...
    f_irreg.add_arc('z_ʧu', 'end', '#', ['ʧ', 'o', 'ʊ', 'z', '#'])
    ...
    f_irreg.add_arc('m_kʌ', 'end', '#', ['k', 'e', 'ɪ', 'm', '#'])
    ...
    f_irreg.add_arc('ɔ_dɹ', 'end', '#', ['d', 'ɹ', 'u', '#'])
    ...
    f_irreg.add_arc('l_fɔ', 'end', '#', ['f', 'ɛ', 'l', '#'])
    ...
    f_irreg.add_arc('ɪ_fla', 'end', '#', ['f', 'l', 'u', '#'])
    ...
    f_irreg.add_arc('d_haɪ', 'end', '#', ['h', 'ɪ', 'd', '#'])
    ...
    '''
    # lie = lay
    f_irreg.add_arc('ɪ_la', 'end', '#', ['l', 'e', 'ɪ', '#'])
    '''
    ...
    f_irreg.add_arc('v_pɹu', 'end', '#', ['p', 'ɹ', 'u', 'v', 'd', '#'])
    ...
    f_irreg.add_arc('d_ɹaɪ', 'end', '#', ['ɹ', 'o', 'ʊ', 'd', '#'])

    # ring = rang
    f_irreg.add_arc('ɹ', 'ɪ_ɹ', 'ɪ', [])
    f_irreg.add_arc('ɪ_ɹ', 'ŋ_ɹɪ', 'ŋ', [])
    f_irreg.add_arc('ŋ_ɹɪ', 'end', '#', ['ɹ', 'æ', 'ŋ', '#'])

    # rise = rose
    f_irreg.add_arc('ɪ_ɹa', 'z_ɹaɪ', 'z', [])
    f_irreg.add_arc('z_ɹaɪ', 'end', '#', ['ɹ', 'o', 'ʊ', 'z', '#'])

    # run = ran
    f_irreg.add_arc('ɹ', 'ʌ_ɹ', 'ʌ', [])
    f_irreg.add_arc('ʌ_ɹ', 'n_ɹʌ', 'n', [])
    f_irreg.add_arc('n_ɹʌ', 'end', '#', ['ɹ', 'æ', 'n', '#'])
    ...
    f_irreg.add_arc('ʊ_so', 'end', '#', ['s', 'o', 'ʊ', 'd', '#'])
    """
    ...
    f_irreg.add_arc('k_ʃeɪ', 'end', '#', ['ʃ', 'ʊ', 'k', '#'])
    ...
    f_irreg.add_arc('k_ʃɹɪŋ', 'end', '#', ['ʃ', 'ɹ', 'æ', 'ŋ', 'k', '#'])
    ...
    # sing = sang
    f_irreg.add_arc('ŋ_sɪ', 'end', '#', ['s', 'æ', 'ŋ', '#'])

    # sink = sank
    f_irreg.add_arc('ŋ_sɪ', 'k_sɪŋ', 'k', [])
    f_irreg.add_arc('k_sɪŋ', 'end', '#', ['s', 'æ', 'ŋ', 'k', '#'])
    ...
    f_irreg.add_arc('k_spi', 'end', '#', ['s', 'p', 'o', 'ʊ', 'k', '#'])
    ...
    f_irreg.add_arc('ŋ_spɹɪ', 'end', '#', ['s', 'p', 'ɹ', 'æ', 'ŋ', '#'])
    ...
    f_irreg.add_arc('l_sti', 'end', '#', ['s','t','o','ʊ','l','#'])
    ...
    f_irreg.add_arc('k_stɪŋ', 'end', '#', ['s','t','æ','ŋ','k','#'])
    ...
    f_irreg.add_arc('d_stɹaɪ', 'end', '#', ['s','t','ɹ','o','ʊ','d','#'])
    ...
    f_irreg.add_arc('ɹ_swɛ', 'end', '#', ['s','w','ɔ','ɹ','#'])
    ...
    f_irreg.add_arc('l_swɛ', 'end', '#', ['s','w','ɛ','l','d','#'])
    ...
    f_irreg.add_arc('m_swɪ', 'end', '#', ['s','w','æ','m','#'])
    ...
    f_irreg.add_arc('k_teɪ', 'end', '#', ['t', 'ʊ', 'k', '#'])
    ...
    f_irreg.add_arc('ɹ_tɛ', 'end', '#', ['t','ɔ','ɹ','#'])
    ...
    f_irreg.add_arc('ʊ_θɹo', 'end', '#', ['θ','ɹ','u','#'])
    ...
    f_irreg.add_arc('k_weɪ', 'end', '#', ['w','o','ʊ','k','#'])
    ...
    f_irreg.add_arc('ɹ_wɛ', 'end', '#', ['w','ɔ','ɹ','#'])
    ...
    f_irreg.add_arc('w__ɹɪŋ_w', 'end', '#', ['ɹ', 'ʌ', 'ŋ', '#'])

    # write = wrote
    f_irreg.add_arc('ɪ_ɹa', 't_ɹaɪ', 't', [])
    f_irreg.add_arc('t_ɹaɪ', 'end', '#', ['ɹ', 'o', 'ʊ', 't'])

    return f_irreg


def final_past_fst(inp):
    f_irreg = past_irreg()
